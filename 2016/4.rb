...
def valid_room?(room)
  ...
  counts = name.chars.group_by(&:itself)
  counts.merge!(counts) { |k, v| v.length }
  counts.delete("-")

  checksum_should_be = counts.keys.sort_by do |letter|
    [-counts[letter], letter]
  end[0, 5].join("")

  checksum_should_be == checksum
end

def decrypt_name!(room)
  name, sector_id, checksum = room

  room[0] = name.chars.map do |c|
    if c == "-"
      "-"
    else
      ("a".ord + ((c.ord - "a".ord + sector_id) % 26)).chr
    end
  end.join("")
end

rooms = input.lines.map do |line|
  case line
  when /^([a-z-]+)-([0-9]+)\[([a-z]{5})\]$/
    [$1, $2.to_i, $3]
  else
...
valid_rooms = rooms.select(&method(:valid_room?))
valid_rooms.each(&method(:decrypt_name!))
p valid_rooms.select { |r| r.first == "northpole-object-storage" }
