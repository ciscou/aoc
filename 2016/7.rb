  ...
  ip.chars.each do |c|
    ...
    if new_slice
      if bracket > 0
        ...
        outside_slices << slice
      end

      slice = ""
    end

    bracket += bracket_diff

    raise ip unless [0, 1].include?(bracket)
  end
...
def abba?(s)
  s.chars.each_cons(4).any? do |a, b, c, d|
    a == d && b == c && a != b
  end
end

def supports_tls?(ip)
  inside_slices, outside_slices = separate_slices(ip)

  inside_slices.none?(&method(:abba?)) && outside_slices.any?(&method(:abba?))
end

def supports_ssl?(ip)
  inside_slices, outside_slices = separate_slices(ip)

  inside_slices.any? do |inside_slice|
    inside_slice.chars.each_cons(3).any? do |a, b, c|
      a == c && outside_slices.any? do |outside_slice|
        outside_slice.chars.each_cons(3).any? do |d, e, f|
          d == f && a == e && b == d
        end
      end
    end
...
ips = input.lines.map(&:chomp!)

puts ips.select(&method(:supports_tls?)).length
puts ips.select(&method(:supports_ssl?)).length
