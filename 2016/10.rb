...
responsabilities = Hash.new { |h, k| h[k] = [] }
outputs = Hash.new { |h, k| h[k] = [] }

instructions = input.lines.map(&:chomp!)

until instructions.empty?
  instructions.reject! do |line|
    raise "uh oh..." if bots.values.any? { |bot| bot.length > 2 }

    case line
    when /^value ([0-9]+) goes to bot ([0-9]+)$/
      bot = bots[$2.to_i]
    ...
    when /^bot ([0-9]+) gives low to (bot|output) ([0-9]+) and high to (bot|output) ([0-9]+)$/
      bot = bots[$1.to_i]

      if bot.length >= 2
        responsabilities[$1.to_i] << bot.sort
        ...
        when "output"
          ...
          "raise unexpected value #{$2.inspect} "
        end
        ...
        when "output"
          ...
          "raise unexpected value #{$4.inspect} "
        end
...
responsabilities.each do |k, v|
  puts k if v.all? { |low, high| low == 17 && high == 61 }
end
