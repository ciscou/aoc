...
def evolve(grid)
  grid.map.with_index do |row, r|
    row.map.with_index do |cell, c|
      evolve_cell(grid, r, c)
    end
  end
end

def evolve_cell(grid, r, c)
  case grid[r][c]
  when "L"
    if neightbours(grid, r, c).count("#") == 0
      "#"
    else
      "L"
    end
  when "#"
    if neightbours(grid, r, c).count("#") >= 4
      "L"
    else
      "#"
    end
...
def evolve2(grid)
  grid.map.with_index do |row, r|
    row.map.with_index do |cell, c|
      evolve_cell2(grid, r, c)
    end
  end
end

def evolve_cell2(grid, r, c)
  case grid[r][c]
  when "L"
    if neightbours2(grid, r, c).count("#") == 0
      "#"
    else
      "L"
    end
  when "#"
    if neightbours2(grid, r, c).count("#") >= 5
      "L"
    else
      "#"
    end
...
def neightbours2(grid, row, col)
  ...
  [
    ...
    begin
      r += dr
      c += dc
      out_of_bounds = r < 0 || r >= grid.length || c < 0 || c >= grid[r].length

      seat = grid[r][c] unless out_of_bounds || grid[r][c] == "."
    end until out_of_bounds || seat
