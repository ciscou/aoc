...
def valid_height?(s)
  case s
  when /\A(\d+)cm\z/ then between?($1.to_i, 150, 193)
  when /\A(\d+)in\z/ then between?($1.to_i,  59,  76)
  else false
  end
end
...
def valid_passport?(passport)
  validations = {
    "byr" => -> (s) { four_digits?(s) && between?(s.to_i, 1920, 2002) },
    "iyr" => -> (s) { four_digits?(s) && between?(s.to_i, 2010, 2020) },
    "eyr" => -> (s) { four_digits?(s) && between?(s.to_i, 2020, 2030) },
    "hgt" => -> (s) { valid_height?(s) },
    "hcl" => -> (s) { s =~ /\A#[0-9a-f]{6}\z/ },
    "ecl" => -> (s) { %w[amb blu brn gry grn hzl oth].include?(s) },
    "pid" => -> (s) { nine_digits?(s) }
  }

  validations.all? do |k, v|
    passport[k] && v.call(passport[k])
  end
end

valid_passports = passports.count { |p| valid_passport?(p) }
