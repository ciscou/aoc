...
passwords = input.lines.map do |line|
  line.chomp!

  spec, password = line.split(": ")
  repeats, letter = spec.split(" ")
  min, max = repeats.split("-").map(&:to_i)

  {min: min, max: max, letter: letter, password: password}
end

def valid_password_old?(password)
  count = password[:password].count(password[:letter])
  password[:min] <= count && count <= password[:max]
end

def valid_password?(password)
  a = password[:password][password[:min] - 1]
  b = password[:password][password[:max] - 1]
  l = password[:letter]
...
p passwords.length
p passwords.count { |p| valid_password?(p) }
