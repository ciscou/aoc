...
input.lines.each do |line|
  ...
  a = a.delete_suffix(" bag").delete_suffix(" bags")
  b = b.split(", ").map { |s| s.delete_suffix(" bag").delete_suffix(" bags") }

  if b == ["no other"]
    $bags[a] = []
  else
    $bags[a] = b.map { |s| n, s = s.split(" ", 2) ; [n.to_i, s] }
  end
end

def contains_at_least_one_of?(a, b)
  $bags[a].any? do |n, x|
    x == b || contains_at_least_one_of?(x, b)
  end
end
...
part1 = $bags.keys.count do |bag|
  contains_at_least_one_of?(bag, "shiny gold")
end
