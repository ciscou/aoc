...
input.lines.each do |line|
  case line
  when /^(\w+) \((\d+)\)$/
    programs[$1] = { name: $1, weight: $2.to_i, children: [] }
  when /^(\w+) \((\d+)\) -> (\w+(, \w+)*)$/
    programs[$1] = { name: $1, weight: $2.to_i }
  else
...
input.lines.each do |line|
  ...
  when /^(\w+) \((\d+)\)$/
  when /^(\w+) \((\d+)\) -> (\w+(, \w+)*)$/
    parent = programs.fetch($1)

    children = $3.split(", ").map { |name| programs.fetch(name) }
...
root = programs.values.reject { |program| program[:parent] }.first

puts root[:name]

def calculate_children_weight(program)
  children = program[:children]

  children.each do |child|
    calculate_children_weight(child)
  end

  program[:children_weight] = children.sum { |child| child[:weight] + child[:children_weight] }
end

def find_unbalanced_node(program)
  children = program[:children]

  return nil if children.length < 2

  weights = children.map { |child| child[:weight] + child[:children_weight] }

  counts = weights.group_by(&:itself)
  counts.merge!(counts) { |k, v| v.length }

  child = children.map { |child| find_unbalanced_node(child) }.compact.first

  return child if child

  if counts.values.sort.uniq.length > 1
    program
  end
end

calculate_children_weight(root)
unbalanced = find_unbalanced_node(root)
puts unbalanced[:name]
res = unbalanced[:children].map { |child| [child[:weight], child[:children_weight], child[:weight] + child[:children_weight]] }
p res
