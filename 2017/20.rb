...
particles = input.lines.map.with_index do |line, index|
  line.chomp!
  ...
  {
    i: index,
    p: p[3..-2].split(",").map(&:to_i),
    v: v[3..-2].split(",").map(&:to_i),
    a: a[3..-2].split(",").map(&:to_i),
  }
end

1000.times do
  particles.each do |particle|
    3.times { |i| particle[:v][i] += particle[:a][i] }
    3.times { |i| particle[:p][i] += particle[:v][i] }
  end

  by_position = particles.group_by { |particle| particle[:p] }
  by_position.delete_if { |k, v| v.length > 1 }

  particles = by_position.values.flatten
end

closest_to_origin = particles.min_by { |particle| particle[:p].map(&:abs).sum }
