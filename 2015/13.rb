...
happy = Hash.new { |h, k| h[k] = Hash.new(0) }
names = []
...
input.lines.each do |line|
  ...
  case line
  when /^(\w+) would gain (\d+) happiness units by sitting next to (\w+).$/
    happy[$1][$3] = $2.to_i

    names << $1
    names << $3
  when /^(\w+) would lose (\d+) happiness units by sitting next to (\w+).$/
    happy[$1][$3] = -$2.to_i
...
highest_happiness = -999_999_999_999

names.permutation.each do |ns|
  happiness = 0

  ([ns.last] + ns + [ns.first]).each_cons(3).each do |n1, n2, n3|
    h1 = happy[n2][n1]
    h3 = happy[n2][n3]
  ...
  highest_happiness = happiness if happiness > highest_happiness
end

puts "highest happiness: #{highest_happiness}"
