...
def turn(lines, val, x1, y1, x2, y2)
  (x1..x2).each do |col|
    (y1..y2).each do |row|
      lines[row][col] += val
      lines[row][col] = 0 if lines[row][col] < 0
    end
  end
end

def toggle(lines, x1, y1, x2, y2)
  (x1..x2).each do |col|
    (y1..y2).each do |row|
      lines[row][col] += 2
    end
...
lights = 1000.times.map do |row|
  1000.times.map do |col|
    0
  end
end
...
input.lines.each do |line|
  ...
  case line
  when /^turn (on|off) (\d+),(\d+) through (\d+),(\d+)$/
    turn(lights, $1 == "on" ? 1 : -1, $2.to_i, $3.to_i, $4.to_i, $5.to_i)
  when /^toggle (\d+),(\d+) through (\d+),(\d+)$/
    toggle(lights, $1.to_i, $2.to_i, $3.to_i, $4.to_i)
  else
    raise "Unknown action #{line.inspect}"
  end
end

lights_on = lights.sum do |row|
  row.sum
end
