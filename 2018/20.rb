...
def follow_directions(grid, directions, x, y)
  idx = 0
  ...
  while idx < directions.length
    ...
    when "("
      ...
      while pars > 0
        ...
        when "|"
          branches << "" if pars == 1
          branches.last << dir unless pars == 1
        when "("
          pars += 1
          branches.last << dir
        when ")"
          pars -= 1
          branches.last << dir unless pars == 0
        end
      ...
      branches.each do |branch|
        follow_directions(grid, branch, x, y)
      end
    end
...
def bfs(grid)
  ...
  until queue.empty?
    ...
    if grid[[x, y-1]] == "-"
      queue.push([x, y-2, doors+1]) unless visited[[x, y-2]]
      visited[[x, y-2]] = true
    end
    if grid[[x, y+1]] == "-"
      queue.push([x, y+2, doors+1]) unless visited[[x, y+2]]
    ...
    if grid[[x-1, y]] == "|"
      queue.push([x-2, y, doors+1]) unless visited[[x-2, y]]
      visited[[x-2, y]] = true
    end
    if grid[[x+1, y]] == "|"
      queue.push([x+2, y, doors+1]) unless visited[[x+2, y]]
...
follow_directions(grid, input[1..-2], 0, 0)
