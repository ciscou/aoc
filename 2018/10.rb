...
stars = input.lines.map do |line|
  ...
  {
    position: position.split(",").map(&:strip).map(&:to_i),
    velocity: velocity.split(",").map(&:strip).map(&:to_i)
  }
end
  ...
  rows.times do |row|
    line = cols.times.map do |col|
      if stars.any? do |s|
        x, y = s[:position]
        vx, vy = s[:velocity]
        [x + vx * ts, y + vy * ts] == [col-cols/2+cx, row-rows/2+cy]
      end
        "#"
      else
        "."
