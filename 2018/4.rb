...
sleeping = Hash.new { |h, guard| h[guard] = Hash.new(0) }
...
input.lines.sort.each do |line|
  ...
  when "falls asleep"
    asleep_at = ts
  when "wakes up"
    asleep_at_min = asleep_at[-2, 2].to_i
    woken_up_min = ts[-2, 2].to_i

    min = asleep_at_min
    while min != woken_up_min
      sleeping[guard][min] += 1
      min += 1
      min %= 60
    end
  else
...
def strategy1(sleeping)
  sleeping.max_by { |guard, stats| stats.values.sum }
end

def strategy2(sleeping)
  sleeping.max_by { |guard, stats| stats.values.max }
end

guard, stats = strategy1(sleeping)
minute = stats.max_by(&:last).first
puts guard * minute

guard, stats = strategy2(sleeping)
minute = stats.max_by(&:last).first
puts guard * minute
