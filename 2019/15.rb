...
class RepairDroidArea
  ...
  attr_reader :path, :best_path, :ox, :oy

  def expand_oxygen
    res = 0

    oxygen_cells = @grid.map do |k, v|
      k if v == "O"
    end.compact

    oxygen_cells.each do |cell|
      x, y = cell
      res += 1 if @grid[[x-1, y]] == "."
      res += 1 if @grid[[x+1, y]] == "."
      res += 1 if @grid[[x, y-1]] == "."
      res += 1 if @grid[[x, y+1]] == "."
      @grid[[x-1, y]] = "O" if @grid[[x-1, y]] == "."
      @grid[[x+1, y]] = "O" if @grid[[x+1, y]] == "."
      @grid[[x, y-1]] = "O" if @grid[[x, y-1]] == "."
      @grid[[x, y+1]] = "O" if @grid[[x, y+1]] == "."
    end
  ...
  def write(val)
    ...
    case val
    when 0 then @grid[[newx, newy]] = "#" ; @path.pop
    when 1 then @grid[[newx, newy]] = "." ; @x = newx ; @y = newy
    when 2 then @grid[[newx, newy]] = "O" ; @x = newx ; @y = newy ; @ox = newx ; @oy = newy ; @best_path = @path.dup
    else raise "invalid val"
    end
  ...
  def read
    ...
    available_directions.reject! { |dir| dir == inverse_direction(@path.last) } unless @path.empty?
    available_directions.reject! { |dir| @breadcrumbs[[@x, @y]][dir] }

    if available_directions.empty?
      @direction = inverse_direction(@path.pop)
    else
      @direction = available_directions.first

      @breadcrumbs[[@x, @y]][@direction] = true
      @path.push(@direction)
    end
  ...
  def display_grid
    80.times do |y|
      line = 120.times.map { |x| display_tile(x-40, y-60) }.join(" ")
      puts line
    end
  end
...
class RepairDroidAreaIntcode < Intcode
  def initialize(source, rda)
    super(source)
